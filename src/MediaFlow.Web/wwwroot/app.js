...
const TITLES = {
  ...
  shares: ['Shares', 'The folders MediaFlow can see. Your sync tool keeps them filled.'],
  ...
  settings: ['Automation & safety', 'How often MediaFlow looks, and what it is allowed to do.'],
...
function applyTheme(theme) {
  ...
  try { localStorage.setItem('mediaflow.theme', theme); } catch {}
}
...
function renderStorage() {
  ...
  if (!items.length) {
    target.innerHTML = `
      <div style="font-size:13px;color:var(--mut);line-height:1.6">
        No destination share configured yet, so there is nowhere for MediaFlow to put anything.
      </div>
      <div class="actions" style="margin-top:14px">
  ...
  if (!primary.exists) {
    target.innerHTML = `
      ...
      <div style="font-size:12.5px;color:var(--mut)">${escapeHtml(primary.error || 'MediaFlow cannot see this folder inside the container.')}</div>
  ...
  // The bar splits the free space into what MediaFlow may use and the reserve
  ...
  target.innerHTML = `
    ...
    <div class="meter" title="Usable free space versus the reserve MediaFlow holds back">
...
function renderOnboarding() {
  ...
  $('onboardingSummary').textContent =
    `${steps.length - remaining} of ${steps.length} steps done. MediaFlow stays in Dry Run until you say otherwise.`;
  ...
  const checks = [
    {
      ...
      detail: 'Run a routing preview on a source share to confirm the capture times MediaFlow reads.',
