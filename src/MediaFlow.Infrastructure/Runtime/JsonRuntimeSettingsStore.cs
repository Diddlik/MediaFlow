...
namespace MediaFlow.Infrastructure.Runtime;

public sealed class JsonRuntimeSettingsStore(
    string path,
    MediaFlowRuntimeSettings defaults) : IRuntimeSettingsStore
{
    private readonly string _path = Path.GetFullPath(path);
    private readonly MediaFlowRuntimeSettings _defaults = Normalize(defaults);
    private readonly SemaphoreSlim _gate = new(1, 1);
    private MediaFlowRuntimeSettings? _cached;

    public async Task<MediaFlowRuntimeSettings> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var loaded = await JsonSerializer.DeserializeAsync<MediaFlowRuntimeSettings>(stream, cancellationToken: cancellationToken);
    ...
    public async Task<MediaFlowRuntimeSettings> UpdateAsync(
    ...
    public async Task<MediaFlowRuntimeSettings> ResetAsync(CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try
        {
