...
namespace MomentFerry.Web.Integrations;
    ...
    ILogger<MqttIntegrationWorker> logger) : BackgroundService
{
    ...
    {
        if (!configuration.GetValue("MomentFerry:Mqtt:Enabled", false))
        {
            logger.LogInformation("MQTT integration is disabled");
            return;
        }

        var host = configuration["MomentFerry:Mqtt:Host"];
        if (string.IsNullOrWhiteSpace(host))
        {
            logger.LogWarning("MQTT integration is enabled but MomentFerry:Mqtt:Host is empty");
            return;
        }

        var port = Math.Clamp(configuration.GetValue("MomentFerry:Mqtt:Port", 1883), 1, 65535);
        var clientId = configuration["MomentFerry:Mqtt:ClientId"];
        if (string.IsNullOrWhiteSpace(clientId)) clientId = "momentferry-" + Environment.MachineName.ToLowerInvariant();

        var baseTopic = NormalizeBaseTopic(configuration["MomentFerry:Mqtt:BaseTopic"] ?? "momentferry");
        ...
        var username = configuration["MomentFerry:Mqtt:Username"];
        var password = configuration["MomentFerry:Mqtt:Password"];
        if (!string.IsNullOrWhiteSpace(username))
            optionsBuilder.WithCredentials(username, password ?? string.Empty);

        if (configuration.GetValue("MomentFerry:Mqtt:UseTls", false))
            optionsBuilder.WithTlsOptions(_ => { });

        var options = optionsBuilder.Build();
        var reconnectSeconds = Math.Clamp(configuration.GetValue("MomentFerry:Mqtt:ReconnectSeconds", 5), 1, 300);
    ...
    private static string NormalizeBaseTopic(string topic) =>
        string.IsNullOrWhiteSpace(topic) ? "momentferry" : topic.Trim().Trim('/');
}
